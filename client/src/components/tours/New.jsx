...
import { toast } from 'react-toastify';
 ...
 const New = function ({user}) {
  ...
  const getTourTypes = async function(){
    const tourType = await Axios.get('/api/tours/tourTypes');

    if(tourType.status === 200){
      setTourTypes(tourType.data);
    }
  };
...
const [redirect, setRedirect] = useState(false);
  const handleSubmit = async event => {
    ...
    try {
      ...
      if (resp.status === 200)  {
        toast("The tour was registered successfully!", {
          type: toast.TYPE.SUCCESS
        });
        setRedirect(true);
      } else {
        toast("There was an issue getting u in.", {
